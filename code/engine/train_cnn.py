...
if __name__ == '__main__':
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=prediction_layer, labels=true_labels)) 
    # TODO: define the loss function
    correct_prediction = tf.equal(tf.argmax(prediction_layer, 1), tf.argmax(true_labels, 1))
    # TODO: define the correct predictions calculation
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32)) 
    # TODO: calculate accuracy
    optimizer = tf.train.AdamOptimizer(learning_rate=0.0005).minimize(cross_entropy)  
